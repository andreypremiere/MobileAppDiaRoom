...
class MediaGrid extends StatelessWidget {
  ...
  void _openFullScreen(BuildContext context, int index) {
    ...
    final isVideo  = att.attType == AttachmentType.video;
    ...
    if (isVideo) {
      ...
      final targetAtt = attachments[index];
      int photoIndex = 0;
      int currentPhotoIndex = 0;
      List<String> photoList = [];
      for (int i = 0; i < attachments.length; i++) {
        final att = attachments[i];
        if (att.attType == AttachmentType.photo) {
          photoList.add(att.s3Key);
          if (identical(targetAtt, att)) {
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: ClipRRect(
        ...
        child: _buildLayout(count, context),
      ),
    );
  }

  Widget _buildLayout(int count, BuildContext context) {
    switch (count) {
      case 1:
        return _item(context, 0, height: 250);
      case 2:
        return Row(
          children: [
            Expanded(child: _item(context, 0, height: 200)),
            const SizedBox(width: 2),
            Expanded(child: _item(context, 1, height: 200)),
          ],
        );
      case 3:
        return Column(
          children: [
            _item(context, 0, height: 180),
            const SizedBox(height: 2),
            Row(
              children: [
                Expanded(child: _item(context, 1, height: 120)),
                const SizedBox(width: 2),
                Expanded(child: _item(context, 2, height: 120)),
              ],
            ),
          ],
        );
          ...
          children: [
            ...
            Row(
              children: [
                ...
                const SizedBox(width: 2),
                Expanded(child: _item(context, 1, height: 180)),
              ],
            ),
            const SizedBox(height: 2),
            ...
            Row(
              children: [
                ...
                const SizedBox(width: 2),
                Expanded(child: _item(context, 3, height: 110)),
                const SizedBox(width: 2),
                Expanded(child: _item(context, 4, height: 110)),
              ],
            ),
          ],
        );
          ...
          children: [
            ...
            Row(
              children: [
                ...
                const SizedBox(width: 2),
                Expanded(child: _item(context, 1, height: 100)),
                const SizedBox(width: 2),
                Expanded(child: _item(context, 2, height: 100)),
              ],
            ),
            const SizedBox(height: 2),
  ...
  Widget _item(BuildContext context, int index, {double? height}) {
    final Attachment att = attachments[index];
    return SizedBox(
      height: height,
      child: Stack(
